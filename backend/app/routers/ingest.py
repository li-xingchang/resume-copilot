...
Flow:
  1. Extract raw text from uploaded file (pdfplumber for PDF, python-docx for DOCX)
  2. LLM parses text into atomic career facts
  3. Embed each fact in batch (one API call to text-embedding-3-small)
  4. Persist to career_facts; return for user verification on /onboard
"""
import io
import uuid
...
async def ingest(
    ...
    # Embed all facts in a single batch API call
